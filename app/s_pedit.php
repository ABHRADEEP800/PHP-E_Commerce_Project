...
<?php
        ...
        $semail=$_SESSION['seller'];
            ...
            $user_id=$row['user_id'];
        $sql="SELECT * FROM product WHERE product_id=$id AND `product_userid`='$user_id'";
        $result=mysqli_query($conn,$sql);
         $row=mysqli_fetch_assoc($result);
        ...
        ?>
<!---Php to update product information--->
<?php
if(isset($_POST["update"])) {
  ...
  $sql= "UPDATE product SET  product_name ='$product_name' , product_price='$product_price',product_description ='$product_description',product_img='$product_img', product_qu='$product_qu' WHERE product_id ='$id';";
      ...
      echo'<script>
      window.location.href="s_pmgmt.php";
      </script>';
}
?>
